...
Dies ist ein OpenKNX-Modul zur Ansteuerung von StatusLED, wie z.B. LEDs auf Tastwippen oder an Präsenzmeldern.

Das Repository findet man unter: https://github.com/openknx/OFM-StatusLED


<!-- DOC HelpContext="Modus" -->
#### **StatusLED Konfiguration**

Hier kann die StatusLED konfiguriert werden.

Unter "Beschreibung" kann eine (ggf. hardwarespezifisch vorbelegte) informative Beschreibung hinterlegt werden.

"Betriebsmodus" schaltet zwischen LED-Funktion und KO-Steuerung um und blendet weitere Konfigurationsmöglichkeiten ein, die in der jeweiligen Kontexthilfe beschrieben sind.
...
Hier können zwei zusätzliche KOs Farbe (RGB-StatusLED) oder Helligkeit (einfarbige StatusLED) und Effekt eingeblendet werden.

"ändern den Zustand AN"  
hier werden die für den Zustand AN hinterlegten Paramter durch den jeweils gültigen Wert geändert. Eine unmittelbaren Effekt hat ein empfangender Wert auf die LED nur dann, wenn diese im Zustand AN ist.  
Somit kann hier mittels der Prio-Steuerung das über die KO definierte Verhalten wieder übersteuert werden.

"ändern direkt die LED"  
hier werden die empfangenen KOs für Farbe/Helligkeit bzw Effekt sofort und unabhängig vom Zustand (AN/AUS/Prio1/Prio2) an der StausLED wirksam.  
Ein danach empfangenes KO Schalten, Prio1 oder Prio2 versezt die LED jedoch wieder in den dadruch gesetzten Zustand mit den in der ETS hinterlegten Parametern.
